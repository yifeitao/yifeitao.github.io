...
要求评分值与目标值同号，如果目标定得更加狭隘一些，可以令评分值直接为$$\{-1, 1\}$$,这是线性回归问题，损失函数使用误差平方（可以做直观的理解，也可以认为样本的误差符合正态分布，从极大似然的角度推导出，这不是本文的重点。）。注意到$$l_{sqr}=(s(\mathbf x)-y)^2=(s(\mathbf x) * y-y^2)^2=(z-1)^2$$，故误差函数是关于绝对评分值$$z$$的二次函数。从图1中，可以看出$$l_{sqr}$$是始终大于$$l_{0-1}$$的，而且当$$l_{sqr}$$比较小时，$$l_{0-1}$$也比较小，所以可以作为$$l_{0-1}$$的近似。
...
由图1可以看出，它也是$$l_{0-1}(z)$$的一种近似。以对数损失函数为优化目标。
...
$$w_m, \phi_m = arg\underset{w,\phi}{min}\sum_{i=1}^Nexp(-z_{mi})$$，其中$$z_{mi}=y_is_m(\mathbf X_i)$$,该式的进一步推导此处不再详述。
